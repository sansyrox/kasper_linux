...
text_to_speech = TextToSpeechV1(
    username=kaspercfg.get('watson.tts.user'),
    password=kaspercfg.get('watson.tts.pass'))
...
def speak_flite_tts(text):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        with open(fd, 'w') as f:
            f.write(text)
        # Call flite tts to reply the response by kasper
        flite_speech_file = os.path.join(
            kaspercfg.get('path.base'), kaspercfg.get('path.flite_speech'))
        logger.debug(
...
def speak_watson_tts(text):
    ...
    :return: None
    """
    voice = kaspercfg.get('watson.tts.voice')
    if (voice == ''):
        lang = kaspercfg.get("language")[0:2]
        if lang == "en":
...
def speak_google_tts(text):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        Speech(text=text, lang=kaspercfg.get("language")).save(mpiii)
        player.say(mpiii)
