...
class kasperAppWindow(Renderer):
    def __init__(self):
        super().__init__()
        builder = Gtk.Builder()
        builder.add_from_file(
            os.path.join(TOP_DIR, "glade_files/kasper_app.glade"))

        self.window = builder.get_object("app_window")
        self.user_text_label = builder.get_object("user_text_label")
        self.kasper_text_label = builder.get_object("kasper_text_label")
        ...
        builder.connect_signals(kasperAppWindow.Handler(self))
    ...
    def receive_message(self, message_type, payload=None):
        if message_type == 'idle':
            ...
            self.kasper_text_label.set_text("")
            ...
            kasper_reply = payload['kasper_reply']
            if 'answer' in kasper_reply.keys():
                self.kasper_text_label.set_text(kasper_reply['answer'])
