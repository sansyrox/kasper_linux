...
import kasper_python as kasper
...
class kasperLoop():
    """The main kasper loop dealing with hotword detection, voice recognition,
    ...
    def __init__(self, renderer=None):
        ...
        self.kasper_config = kasperConfig()
        self.lang = self.kasper_config.get('language')
        self.path_base = self.kasper_config.get('path.base')
        self.sound_detection = os.path.abspath(
                                   os.path.join(self.path_base,
                                                self.kasper_config.get('path.sound.detection')))
        self.sound_problem = os.path.abspath(
                                   os.path.join(self.path_base,
                                                self.kasper_config.get('path.sound.problem')))
        self.sound_error_recognition = os.path.abspath(
                                   os.path.join(self.path_base,
                                                self.kasper_config.get('path.sound.error.recognition')))
        self.sound_error_timeout = os.path.abspath(
                                   os.path.join(self.path_base,
                                                self.kasper_config.get('path.sound.error.timeout')))

        if self.kasper_config.get('kasper.mode') == 'authenticated':
            try:
                kasper.sign_in(email=self.kasper_config.get('kasper.user'),
                             password=self.kasper_config.get('kasper.pass'))
            except Exception as e:
                logger.error('Some error occurred in login. Check you login details with kasper-config.\n%s', e)

        if self.kasper_config.get('hotword.engine') == 'Snowboy':
            from .hotword_engine.snowboy_detector import SnowboyDetector
            hotword_model = "kasper.pmdl"
            if self.kasper_config.get('hotword.model'):
                logger.debug("Using configured hotword model: " + self.kasper_config.get('hotword.model'))
                hotword_model = self.kasper_config.get('hotword_model')
            self.hotword_detector = SnowboyDetector(model=hotword_model)
        elif self.kasper_config.get('hotword.engine') == 'PocketSphinx':
            from .hotword_engine.sphinx_detector import PocketSphinxDetector
            self.hotword_detector = PocketSphinxDetector()
        elif self.kasper_config.get('hotword.engine') == 'None':
            self.hotword_detector = None
        else:
            raise ValueError(f"Unrecognized value for hotword.engine: {self.kasper_config.get('hotword.engine')}")

        if self.kasper_config.get('wakebutton') == 'enabled':
            logger.info("kasper has the wake button enabled")
            if self.kasper_config.get('device') == 'RaspberryPi':
                logger.info("kasper runs on a RaspberryPi")
                from .hardware_components.rpi_wake_button import RaspberryPiWakeButton
                self.wake_button = RaspberryPiWakeButton()
            else:
                logger.warning("kasper is not running on a RaspberryPi")
                self.wake_button = None
        else:
            logger.warning("kasper has the wake button disabled")
            self.wake_button = None


        stt = self.kasper_config.get('stt')
        ...
        if self.kasper_config.get('stt') == 'deepspeech-local':
    ...
    def server_checker(self):
        ...
        while response_one is None:
            try:
                ...
                url = urljoin(self.server_url, '/kasper/chat.json')
                response_one = requests.get(url, test_params).result()
                api_endpoint = self.server_url
                kasper.use_api_endpoint(api_endpoint)
    ...
    def __speak(self, text):
        ...
        tts = self.kasper_config.get('tts')
        if tts == 'google':
    ...
    def recognize_audio(self, recognizer, audio):
        ...
        stt = self.kasper_config.get('stt')
        lang = self.kasper_config.get('language')
        ...
        if stt == 'google':
            ...
            username = self.kasper_config.get('watson.stt.user')
            password = self.kasper_config.get('watson.stt.pass')
            ...
            api_key = self.kasper_config.get('bing.api')
    ...
    def deal_with_error(self, payload=None):
        ...
        if payload == 'RecognitionError':
            ...
            self.kasper_config.set('tts', 'flite')
            self.kasper_config.set('stt', 'pocketsphinx')
    ...
    def deal_with_answer(self, payload=None):
        ...
        try:
            ...
            if isinstance(payload, str):
                logger.debug("Sending payload to kasper server: %s", payload)
                reply = self.kasper.ask(payload)
            else:
            ...
            self.notify_renderer('speaking', payload={'kasper_reply': reply})
            ...
            # {'stop': <kasper_python.models.StopAction object at 0x7f4641598d30>}
            if 'stop' in reply.keys():
                no_answer_needed = True
                player.stop()

            if 'answer' in reply.keys():
                logger.info('kasper: %s', reply['answer'])
                lights.off()
                lights.speak()
            ...
            if 'language' in reply.keys():
                ...
                if answer_lang != self.kasper_config.get("language"):
                    logger.info("Switching language to: %s", answer_lang)
                    # switch language
                    self.kasper_config.set('language', answer_lang)
                    # TODO
