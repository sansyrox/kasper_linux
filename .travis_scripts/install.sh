...
echo "Downloading dependency: kasper Python API Wrapper"
if [ ! -d "kasper_python" ]
then
    git clone https://github.com/fossasia/kasper_api_wrapper.git

    echo "setting correct location"
    mv kasper_api_wrapper/python_wrapper/kasper_python kasper_python
    mv kasper_api_wrapper/python_wrapper/requirements.txt requirements.txt
    rm -rf kasper_api_wrapper
fi
...
pip3 install -r requirements.txt  # This is from kasper_api_wrapper
